            ...
            <div className="space-y-1.5">
              ...
              <Controller control={control} name="patientId" render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent>
                    {patients.slice(0, 20).map((p) => <SelectItem key={p.id} value={p.id}>{p.firstName} {p.lastName} — {p.mrn}</SelectItem>)}
                  </SelectContent>
                </Select>
              )} />
            ...
            <div className="space-y-1.5">
              ...
              <Controller control={control} name="insuranceProvider" render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent>
                    {insuranceProviders.map((p) => <SelectItem key={p.id} value={p.name}>{p.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              )} />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Controller control={control} name={`lines.${idx}.cptCode`} render={({ field: f }) => (
                      <Select value={f.value} onValueChange={(v) => {
                        f.onChange(v);
                        const cpt = procedureCodes.find((c) => c.cptCode === v);
                        if (cpt) {
                          const cur = watch(`lines.${idx}`);
                          // @ts-ignore
                          setValueLines(idx, 'unitCharge', cpt.standardCharge);
                        }
                      }}>
                        <SelectTrigger><SelectValue placeholder="Select CPT" /></SelectTrigger>
                        <SelectContent>
                          {procedureCodes.map((c) => <SelectItem key={c.id} value={c.cptCode}>{c.cptCode} — {c.description.slice(0, 30)}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    )} />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Controller control={control} name={`lines.${idx}.icd10Code`} render={({ field: f }) => (
                      <Select value={f.value} onValueChange={f.onChange}>
                        ...
                        <SelectContent>
                          {diagnosisCodes.map((c) => <SelectItem key={c.id} value={c.icd10Code}>{c.icd10Code} — {c.description.slice(0, 30)}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    )} />
                  </div>
...
function setValueLines(idx: number, key: string, value: any) {
  // helper to set unit charge from CPT selection
